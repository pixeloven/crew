...
- **Register each local skill for every harness the project uses.** `.claude/skills/<name>.md`, `.pi/skills/<name>/SKILL.md`, `.agents/skills/<name>/SKILL.md` — single-source the file and symlink the rest. A skill in the wrong layout is invisible to that harness with no error, so confirm placement rather than assuming it.
