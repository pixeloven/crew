---
name: <project>-vault-ops
description: <Project>'s knowledge-base operations — the lint/promote job names and schedules, corpus conventions, the extraction model, and where the vault lives. Load when curating the corpus or debugging KB automation. Pair it with the project's own knowledge-base tooling and curation skills.
---

## Jobs

> **▸ Fill:** the lint and promote job names, schedules, and how to trigger a one-off run.

## Corpus conventions

> **▸ Fill:** note-kind conventions, directory layout, tagging/MOC norms, and the location of the corpus's own conventions note if it keeps one.

## Extraction

> **▸ Fill:** the configured extraction model and any env vars that select it.
