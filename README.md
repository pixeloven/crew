...
**7 role agents**, single-sourced in `roles/<role>/` as exactly two files — a harness-neutral `role.yml` and one shared `body.md` — and rendered by `scripts/render_roles.py` into `agents/*.md` (Claude Code) and `pi-agents/*.md` (pi): `lead`, `triage`, `investigator`, `researcher`, `responder`, `reviewer`, `implementer`. The two trees are **byte-identical below the frontmatter**; only the dialect differs, and the renderer owns that translation. A role declares **who it is and what it may touch** — never a model, a reasoning level, or a turn budget, which are the dispatching session's to choose, and which the renderer rejects by name. Edit `roles/`, never the rendered trees — CI fails on drift.
